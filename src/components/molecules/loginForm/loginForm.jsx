...
const LoginForm = () => {
    ...
    const CLEAN_ERROR_MESSAGE = () => {
        if (userName === '' || userName === null) { setUserNameError("") }
        if (passWord === '' || passWord === null) { setPassWordError("") }
        if (remember === false || remember === null) { setremenberError("") }
    }
    /*--------Method for set the input value in the state--------------*/
    ...
    const VALIDATE_FORM = () => {
        var emailValidation = /^[a-zA-Z0-9.-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*$/;
    ...
    const SEND_DATA = event => {
        ...
        if (VALIDATE_FORM() === true) {
            console.log("PENDIENTE CREAR SERVICIO CON NODE JS");
        }
    };
